...
@Component({
  selector: 'app-kpi-chart-user-average-duration-for-correspondence-completion',
  imports: [
    CommonModule,
    TranslateModule,
  ...
  templateUrl: './kpi-chart-user-average-duration-for-correspondence-completion.component.html',
  styleUrl: './kpi-chart-user-average-duration-for-correspondence-completion.component.scss'
})
export class KpiChartUserAverageDurationForCorrespondenceCompletionComponent implements OnChanges, OnInit {
  @Input() year!: number;
  ...
  loadChartData() {
    forkJoin({
      averageDuration: this.kpiService.GetAverageDurationForCorrespondenceCompletionV3(this.structureId, this.year, this.userId),
